...
def simulate_pcr(
        templates: DNAFASTAFormat, experiments: IPCRessExperimentFormat,
        seed: int = 12, memory: int = 32, mismatch: int = 0,
) -> (DNAFASTAFormat, pd.DataFrame):
    cmd = [
        'ipcress', '-i', str(experiments), '-s', str(templates), '-p', '-P',
        '-S', str(seed), '-M', str(memory), '-m', str(mismatch)
    ]
    ...
    for line in lines:
        ...
        product['start_position'] = int(line[16].split('start')[1].strip().split(' ')[0])
